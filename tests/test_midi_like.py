        ...
        for i, path in enumerate(paths):
            ...
            for include_metric_weights in (True, False):
                ...
                for j in encoded.note_on_idx_to_repr_idx.values():
                    assert (
                        rv["input"][j].startswith("note_on") or rv["input"][j] == "bar"
                    )
                ...
                no_zero_len_note_df = df[(df.type != "note") | (df.onset != df.release)]
                ...
                for window_len, hop in it.product(
                    ...
                    for i, (segment) in enumerate(
                        ...
                        for segment_i, repr_i in enumerate(segment["repr_indices"]):
                            ...
                            if repr_i is None:
                                continue
                            assert encoded.events[repr_i] == segment["input"][segment_i]
                        ...
                        bars = [event for event in segment["input"] if event == "bar"]
                        note_offs = [
                            event
                            for event in segment["input"]
                            if event.startswith("note_off")
